...
//#define MTR_PWM     0
#define LEFT_M0     15  //MOTOR DERECHA
#define RIGHT_M0    13  //MOTOR DERECHA
#define LEFT_M1     14  //MOTOR IZQUIERDA
#define RIGHT_M1    2   //MOTOR IZQUIERDA
#define LEFT_M2     12  //MOTOR TRASERO
#define RIGHT_M2    0   //MOTOR TRASERO
...
volatile unsigned long move_interval = 100;
    ...
    <br>
            ...
            <section id="buttons">
                ...
                <div id="controls" class="control-container">
                  <table>
                  <tr><td align="center"><button class="button button6" id="get-still">Image</button></td><td align="center"><button id="toggle-stream">Iniciar video</button></td><td></td></tr>
                  <tr><td></td><td align="center"><button class="button button2" id="forward" onclick="fetch(document.location.origin+'/control?var=car&val=1');">ADELANTE</button></td><td></td></tr>
                  <tr><td align="center"><button class="button button2" id="turnleft" onclick="fetch(document.location.origin+'/control?var=car&val=2');">IZQUIERDA</button></td><td align="center"></td><td align="center"><button class="button button2" id="turnright" onclick="fetch(document.location.origin+'/control?var=car&val=4');">DERECHA</button></td></tr>
                  <tr><td></td><td align="center"><button class="button button2" id="backward" onclick="fetch(document.location.origin+'/control?var=car&val=5');">ATRÁS</button></td><td></td></tr>
                  <tr><td></td><td align="center"><button class="button button4" id="flash" onclick="fetch(document.location.origin+'/control?var=flash&val=1');">ENCENDER</button></td><td></td></tr>
                  <tr><td></td><td align="center"><button class="button button4" id="flashoff" onclick="fetch(document.location.origin+'/control?var=flashoff&val=0');">APAGAR</button></td><td></td></tr>
                  </table>
                </div>
               <br>
...
void robot_setup()
{
  ...
  // Pins for Motor Controller uses PWM (DON'T USE PWM CHANNEL 0. IT'S USED BY CAMERA MODULE AND DON'T USE PWM CHANNEL 7 IN GPIO16)
  ledcSetup(2,freq, 8);     //pinMode(LEFT_M0,OUTPUT);
  ledcSetup(3,freq, 8);     //pinMode(RIGHT_M0,OUTPUT);
  ledcSetup(4,freq, 8);     //pinMode(LEFT_M1,OUTPUT);
  ledcSetup(5,freq, 8);     //pinMode(RIGHT_M1,OUTPUT);
  ledcSetup(6,freq, 8);     //pinMode(LEFT_M2,OUTPUT);
  ledcSetup(7,freq, 8);     //pinMode(RIGHT_M2,OUTPUT);
  
  ledcAttachPin(LEFT_M0, 2);    // PWM Channel 0 -> control LEFT_M0
  ledcAttachPin(RIGHT_M0, 3);    // PWM Channel 2 -> control RIGHT_M0
  ledcAttachPin(LEFT_M1, 4);    // PWM Channel 3 -> control LEFT_M1
  ledcAttachPin(RIGHT_M1, 5);    // PWM Channel 4 -> control RIGHT_M1
  ledcAttachPin(LEFT_M2, 6);    // PWM Channel 5 -> control LEFT_M2
  ledcAttachPin(RIGHT_M2, 7);    // PWM Channel 6 -> control RIGHT_M2
...
void robot_stop()
{
  ledcWrite(2, 0);              //PWM (Channel 0) value = 0
  ledcWrite(3, 0);              //PWM (Channel 2) value = 0  
  ledcWrite(4, 0);              //PWM (Channel 3) value = 0
  ledcWrite(5, 0);              //PWM (Channel 4) value = 0
  ledcWrite(6, 0);              //PWM (Channel 5) value = 0
  ledcWrite(7, 0);              //PWM (Channel 6) value = 0  
}

void robot_fwd()
{
  ledcWrite(2, 0);           //PWM (Channel 0) value = 0          //digitalWrite(LEFT_M0,LOW);
  ledcWrite(3, speed);       //PWM (Channel 1) value = speed      //digitalWrite(RIGHT_M0,HIGH);
  ledcWrite(4, speed);       //PWM (Channel 2) value = speed      //digitalWrite(LEFT_M1,HIGH);
...
void robot_back()
{
  ledcWrite(2, speed);      //digitalWrite(LEFT_M0,HIGH);
  ledcWrite(3, 0);         //digitalWrite(RIGHT_M0,LOW);  
  ledcWrite(4, 0);          //digitalWrite(LEFT_M1,LOW);
...
void robot_right()
{
  ledcWrite(2, speed);   //digitalWrite(LEFT_M0,HIGH);
  ledcWrite(3, 0);      //digitalWrite(RIGHT_M0,LOW);
  ledcWrite(4, speed);   //digitalWrite(LEFT_M1,HIGH);
...
void robot_left()
{
  ledcWrite(2, 0);          //digitalWrite(LEFT_M0,LOW);
  ledcWrite(3, speed);     //digitalWrite(RIGHT_M0,HIGH);
  ledcWrite(4, 0);      //digitalWrite(LEFT_M1,LOW);
  ledcWrite(5, speed);     //digitalWrite(RIGHT_M1,HIGH);
  ledcWrite(6, 0);          //digitalWrite(LEFT_M2,LOW);
